...
public class MovementService {
    ...
    public int getNextRoom(int roomID, String direction, boolean requirementsMet) {
        return switch (roomID) {
            case 0 -> {
                if (direction.equals("NORTH") && requirementsMet == true) yield 1;
            ...
            case 4 -> {
                if (direction.equals("SOUTH")) yield 5;
                else if (direction.equals("EAST")) yield 3;
                else yield 4;
            }
            case 5 -> {
                if (direction.equals("WEST")) yield 4;
                else if (direction.equals("NORTH")) yield 4;
                else yield 5;
            }
            case 11 -> {
                if (direction.equals("NORTH")) yield 0;
                else yield 11;
            }
            default -> roomID;
        };
